...
- **AIS Passive Income Extraction** — The AIS JSON parser has been upgraded. In addition to extracting capital gains (SFT-017, SFT-018), it now explicitly extracts Salary (192), Dividend (SFT-014), Savings Bank Interest (SFT-016), Deposit Interest (SFT-015), and Rental Income (194I). These are presented in a dedicated interactive grid with an "Accept & Auto-fill" button that automatically injects them into your profile.
- **Built-in AIS Auto-Decryption** — Directly upload the encrypted AIS `.json` or `.zip` downloaded from the IT portal without needing the "Offline Utility". The app now provides a simple UI to enter your PAN and DOB, automatically performing secure on-the-fly PBKDF2-AES decryption to unlock and parse your file.
- **ICICI PDF Parser Upgrade** — Enhanced the ICICI Direct parser to support their new "Summary of Income and Tax Statements" format. It now precisely extracts LTCG, STCG for both Equity and Bonds directly from the summary table, and uniquely extracts Fixed Deposit and Corporate Bond **Interest**. Detected interest triggers a prominent UI alert prompting users to manually enter it in the Deductions & Passive Income section.
...
### Fixed
- **Anand Rathi auto-detect failure** — Broadened the regex in the Anand Rathi auto-detect logic to look for domain markers (`@rathi.com`) rather than hardcoded client name strings, which caused legitimate Anand Rathi statements to fall back to the generic unrecognised PDF handler.
